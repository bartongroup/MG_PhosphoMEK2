...
q_numbers <- function(pho, pep, pro) {
  map_dfr(list(pho, pep, pro), function(x) {
    tibble(n = n_quantified(x))
  }) %>%
...
#'   columns)
RAS <- function(K, max_iter = 50, eps = 1e-5) {
  n <- ncol(K)
  m <- nrow(K)
  ...
  good_rows <- which(rowSums(!is.na(K)) > 0)
  K <- K[good_rows, ]

  cnt <- 1
  repeat {
    row_mult <- 1 / (n * rowMeans(K, na.rm = TRUE))
    K <- K * row_mult
    err1 <- 0.5 * sum(abs(colMeans(K, na.rm = TRUE) - 1 / n))
    col_mult <- 1 / (n * colMeans(K, na.rm = TRUE))
    K <- t(t(K) * col_mult)
    err2 <- 0.5 * sum(abs(rowMeans(K, na.rm = TRUE) - 1 / n))
    cnt <- cnt + 1
    if (cnt > max_iter || (err1 < eps && err2 < eps)) break
  }
