...
plot_sample_distirbutions <- function(set, what = "value_norm", bins = 50,
                                      log_scale = FALSE, ncol = 3) {
  d <- set$dat %>%
    drop_na() %>%
    mutate(val = get(what))
...
plot_clustering <- function(set, what = "value_med", text.size = 10) {
  ...
  corr_mat <- cor(tab, use = "complete.obs")
  dis <- as.dist(1 - corr_mat)  # dissimilarity matrix
  hc <- hclust(dis)
...
plot_volcano <- function(res, fc = "logFC", p = "PValue", fdr = "FDR",
                         groupvar = "contrast", fdr_limit = 0.05, logfc_limit = 0,
                         point_size = 0.5, point_alpha = 0.5) {
  res %>%
    mutate(
      x = get(fc),
      y = get(p),
      sig = get(fdr) < fdr_limit & abs(get(fc)) > logfc_limit
    ) %>%
...
plot_ma <- function(res, fc = "logFC", sm = "AveExpr", fdr = "FDR",
                    groupvar = "contrast", fdr_limit = 0.05, logfc_limit = 0,
                    with_labels = FALSE, label_size = 3, id = "gene_name",
                    point_size = 0.5, point_alpha = 0.5, sel_genes = NULL) {
  res <- res %>%
    mutate(sig = get(fdr) < fdr_limit & abs(get(fc)) > logfc_limit)
  ...
  if (!is.null(sel_genes)) {
    g <- g +
      geom_point(data = filter(res, res$gene_name %in% sel_genes),
                 colour = "red", size = point_size * 1.5)
  }
  if (with_labels) {
    g <- g +
      geom_text_repel(data = filter(res, sig),
                      aes_string(x = sm, y = fc, label = id), size = label_size)
  }
  g
}

plot_up_down <- function(res, fdr_limit = 0.05, logfc_limit = 1, groupvar = "contrast") {
  res %>%
  ...
  d %>%
    ...
    ggplot(dp, aes(x = as_factor(pos), y = logFC)) +
      ...
      geom_col(position = position_dodge(),
               aes(fill = contrast, colour = factor(FDR < 0.05, levels = c(FALSE, TRUE)))) +
      ...
      scale_y_continuous(expand = expansion(mult = c(0.1, 0.1)), limits = c(-mx, mx),
...
plot_pho_vs_prot <- function(pho, pro, sites, ncol = 6) {
  pho$dat %>%
    ...
    left_join(pho$metadata, by = "sample") %>%
    left_join(pho$phospho2prot, by = "id") %>%
    left_join(select(pro$dat, protein_id = id, sample, prot = value_med), by = c("protein_id", "sample")) %>%
