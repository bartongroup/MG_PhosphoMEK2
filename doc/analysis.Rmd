...
We filter all data by removing reverse and potential contaminants. Also we filter `Razor + unique peptides > 2` for proteins and `Localization prob > 0.95` for phospho sites.
...
[Download DE results](http://www.compbio.dundee.ac.uk/user/mgierlinski/phosphomek/tab/de_peptides.tsv)
...
```{r prot_vs_pho_de, fig.width=12, fig.height=14}
tar_read(fig_pho_vs_pro_median) 
```

Heatmap (hierarchical clustering of rows and columns) of significant sites.

```{r de_heatmap, fig.width=8, fig.height=10}
tar_read(fig_de_heatmap)
```

## Resources

- [Interactive data explorer](https://shiny.compbio.dundee.ac.uk/private//marek_phosphomek2/pep/)
