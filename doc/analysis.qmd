...
This is because intensity is measured for the entire peptide ion, while phosphorylation sites are discriminated later, from fragmentation spectra. The probability score is a good indicator of how confident this identification is. Though duplicated intensities make phosphosites dependent (not independent in statistical sense) and can affect the p-values from the differential expression, the effect on DE results is miniscule. Therefore, in the second run of this analysis we decided not to remove duplicates, so they are all reported. 
...
In PD output there are multiple close sites quantified together, for example `Q9UPT8 2xPhospho [S1269(99.9); S1275(100)]` corresponds to one row of measurements, but refers to two close sites. 

The relation between phospho sites, their quantification, peptides and proteins is complex. Have a look at this example. We ave a these two close phospho sites `Q9UPT8:S1269` and `Q9UPT8:S1275`. Proteome Discoverer provides 6 measurements with 6 different modification states of the peptide:

```{r pd_example_pd}
pd_mq_comp$pd_info %>% 
  select(peptide_id, sequence, modifications, modifications_prot, gene) %>%
  flextable() %>% 
  width(j = "modifications", width = 3) %>% 
  width(j = "modifications_prot", width = 3) 
```

4 of these measurements include S1269, 4 include S1275 and 3 include both.

In MaxQuant there are two sites, with four corresponding peptides each and two multiplicity measurements.

```{r pd_example_mq}
pd_mq_comp$mq_info %>%
  select(id, peptide_ids, proteins, gene_name, sequence_window, position) %>% 
  flextable()
```

This table shows the MaxQuant peptide annotations:

```{r pd_example_pep}
pd_mq_comp$pep_info %>% 
  select(id, sequence, protein, gene_name, start_position, end_position) %>% 
  flextable()
```

W see these are the same four sequences as identified with PD.

Here is a comparison of intensities from both data sets:

```{r pd_example_plot, fig.width=9, fig.height=4}
pd_mq_comp$plot
```

So, for this one particular phospho site (with its very close neighbour), if we use both MaxQuant and PD, we have 8 different measurements in each sample. Sigh.
...
-   [Interactive data explorer](https://shiny.compbio.dundee.ac.uk/mgierlinski/public/phosphomek2/de)
